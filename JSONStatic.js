 ...
 * @LastEditTime: 2021-06-30 11:42:04
 */
...
export default function JSONStatic() {
  ...
  JSON.delKey = function delKey(json, rmkey) {
    ...
    let _rmkey =
      arguments.length < 2 ? [] : Array.isArray(rmkey) ? rmkey : [rmkey];
  ...
  JSON.delKeys = function delKeys(json, rmkey, depth) {
    if (
      (isType(json) !== "Object" && isType(json) !== "Array") ||
      typeof rmkey === "undefined"
    )
      return json;
  ...
  JSON.delByVals = function delByVals(json, rmval, depth) {
    ...
    vals.forEach((item, index) => {
      if (isType(item) === "Object" || isType(item) === "Array") {
        !_rmObj.some((element) => compare(item, element)) &&
          (n[keys[index]] = depth ? delByVals(item, _rm, depth) : item);
      } else {
